...
То же самое при добавлении НОВОГО поля в `mapping.go`: `EnsureIndex` на
существующем индексе — no-op (см. `internal/platform/es/client.go`), он не
досылает маппинг. Индексер начинает писать поле в документах, OpenSearch
заводит его динамически — `text` со `standard` анализатором, без `ru_en`
и без синонимов. Внешне: поле вроде ищется, но только точным написанием.
Так «Монтажёр» перестал находиться по запросу «монтажер» — вместо этого
срабатывал broadening и на «монтажер» первым показывался актёр.

Отдельно: документы, проиндексированные ДО появления поля, так и остаются
без него, пока владелец не сохранит профиль (outbox дёргается только на
изменение). После добавления поля переиндексация нужна ещё и поэтому.

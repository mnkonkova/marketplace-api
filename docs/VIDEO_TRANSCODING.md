...
ffmpeg -y \
  ...
  -an \                                  # звук не нужен (autoplay-мьют всё равно)
...
- 480p × 8 сек × CRF 28 ≈ 400-700 KB (зависит от движения).
- Для статичных кадров — 200-300 KB.
- Hard cap: `-fs 800k` если перепрыгивает — отдельный пас с CRF 32.
